...
TR_ID_TO_TABLE: dict[str, str] = {
    ...
    for spec in QUOTATIONS_API_SPECS
    if spec.get("tr_id")
}
...
def _infer_default(element: str, description: str) -> str:
    ...
    if elem in {"F_DT", "FID_INPUT_DATE_1", "FID_INPUT_DATE_1"}:
        return (datetime.now() - timedelta(days=7)).strftime("%Y%m%d")

    if elem in {"T_DT", "FID_INPUT_DATE_2"}:
    ...
    if "INPUT_JISU_SCRN_CD" in elem:
        return "KOSPI"

    if "PERIOD_DIV_CODE" in elem:
        return "D"

    if "TR_CONT" in elem:
        return ""

    if "RANK_SORT" in elem or "SORT" in elem:
        return "0"

    if "DIV_CLS_CODE" in elem:
        return "0"

    if "INPUT_CNT" in elem:
        return "100"

    if "INPUT_HOUR_1" in elem:
        return "090000"

    if "INPUT_HOUR_2" in elem:
        return "153000"

    if "PRICE" in elem or "VOL" in elem or "QTY" in elem or "AMT" in elem or "PBMN" in elem:
        return "0"
...
def _normalize_date_or_time(value: str, element: str) -> str:
    digits = re.sub(r"\D", "", value or "")
    upper = element.upper()
    if "DATE" in upper or upper in {"F_DT", "T_DT"}:
        return digits if len(digits) == 8 else value
    if "HOUR" in upper or "TIME" in upper:
        return digits if len(digits) in {4, 6} else value
    return value
...
def _ensure_missing_field_param(params: dict[str, str], missing_field: str) -> bool:
    ...
    value = _normalize_date_or_time(value, target_key)
...
def call_quotations_api(token: str, spec: dict[str, Any], params: dict[str, str]) -> dict[str, Any]:
    cfg = load_config()
...
def save_quotations_result(spec: dict[str, Any], params: dict[str, str], result: dict[str, Any]) -> int:
...
def print_quotations_api(token: str, spec: dict[str, Any]) -> None:
    print(f"\n[{spec['name']}]")
    ...
    for field in spec.get("fields", []):
        ...
        if not value:
            value = default
        value = _normalize_date_or_time(value, element)
    ...
    retry_count = 0
    while True:
        result = call_quotations_api(token, spec, params)
    ...
    try:
        save_quotations_result(spec, params, result)
    except Exception:
        pass
...
def run_quotations_menu(token: str) -> None:
    specs = QUOTATIONS_API_SPECS

    while True:
        print("\n" + "=" * 84)
        print("[국내주식 Quotations API 메뉴]")
        print("=" * 84)
        ...
        spec = specs[sel - 1]
        try:
            print_quotations_api(token, spec)
        except requests.HTTPError as exc:
            print(f"\n[오류] HTTP 요청 실패: {exc}")
            if exc.response is not None:
                print(exc.response.text)
        except Exception as exc:
            print(f"\n[오류] quotations 조회 실패: {exc}")
